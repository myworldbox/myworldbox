...
var motto = [];
var contentArr = [[], [], []];
var filePath, template, container, operator;
var card;
var html;
...
function fetchJson(filePath, template, container, operator) {
    ...
    fetch(filePath)
        ...
        .then(function (data) {
        var temp = data.map(function (user) {
...
searchInput.addEventListener("input", function (e) {
    ...
    users.forEach(function (user) {
        var isVisible = (user.content0.toLowerCase().includes(value) || user.content2.toLowerCase().includes(value));
