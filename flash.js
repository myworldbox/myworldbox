...
class FullscreenCanvas {
  ...
  renderIntoView(target = document.getElementById("flash")) {
    const { canvas } = this;
...
const shape = {
  ...
  init(container = document.getElementById("flash")) {
    if (this.isAlive) {
      return;
    }
...
/*
document.getElementById('flashInput').addEventListener('keypress', ({ keyCode, target }) => {
  if (keyCode === 13) {
    clearTimeout(timer);
